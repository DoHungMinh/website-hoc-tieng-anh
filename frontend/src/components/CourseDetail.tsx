...
const CourseDetail: React.FC<CourseDetailProps> = ({
    ...
    const isEnrolled =
        enrollments?.some(
            (enrollment) =>
                enrollment?.courseId && enrollment.courseId._id === course.id
        ) || false;
    ...
    const createPayOSPayment = async () => {
        ...
        try {
            ...
            console.log(
                "API URL:",
                `${import.meta.env.VITE_API_URL}/payos/create-payment`
            );
    ...
    // Check payment status với cleanup mechanism
    const startPaymentStatusCheck = (orderCode: number) => {
        let timeoutId: NodeJS.Timeout | null = null;
        let attempts = 0;
        const maxAttempts = 20; // Tối đa 20 lần check (60 giây)

        const checkStatus = async () => {
            // Clear timeout nếu đã vượt quá số lần cho phép
            if (attempts >= maxAttempts) {
                console.log("Payment status check timeout after 60 seconds");
                return;
            }
            ...
            // Continue checking every 3 seconds if not paid yet and under limit
            if (attempts < maxAttempts) {
        ...
        // Return cleanup function (although not used in current code structure)
        return () => {
            if (timeoutId) {
    ...
    const handlePaymentSuccess = async (orderCode: number) => {
        try {
            ...
            const errorMessage =
                error instanceof Error
                    ? error.message
                    : "Đã xảy ra lỗi xử lý thanh toán";
                            ...
                            <div className="max-w-2xl mx-auto">
                                ...
                                <div className="mb-8">
                                    ...
                                    <p className="text-xs text-gray-500 text-center mt-4">
                                        Bằng cách nhấn "Thanh toán bằng QR
                                        PayOS", bạn đồng ý với Điều khoản sử
                                        dụng và Chính sách bảo mật của chúng tôi
                                    </p>
                                </div>
                            </div>
                        </div>
                                ...
                                {!isPurchased && !showPreview ? (
                                    <div className="text-center py-8 bg-gray-50 rounded-lg">
                                        ...
                                        <p className="text-gray-600 mb-2">
                                            Nội dung từ vựng sẽ được mở khóa sau
                                            khi mua khóa học
                                        </p>
                                        <p className="text-sm text-gray-500">
                                            {course.vocabulary.length} từ vựng
                            ...
                            <div className="bg-white rounded-xl shadow-sm p-6">
                                ...
                                {!isPurchased && !showPreview ? (
                                    <div className="text-center py-8 bg-gray-50 rounded-lg">
                                        <BookOpen className="w-12 h-12 mx-auto text-gray-400 mb-3" />
                                        <p className="text-gray-600 mb-2">
                                            Nội dung ngữ pháp sẽ được mở khóa
                                            sau khi mua khóa học
                                        </p>
                                        <p className="text-sm text-gray-500">
                                            {course.grammar.length} nội dung ngữ
                                            pháp đang chờ bạn khám phá
                                        </p>
                                    </div>
                                ) : (
                                    <div className="relative">
