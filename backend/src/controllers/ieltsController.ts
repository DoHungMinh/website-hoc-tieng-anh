...
const updateUserLevelFromTestResults = async (userId: string) => {
    try {
        ...
        const newLevel = calculateUserLevel(
            testResults.map((result) => ({ bandScore: result.score.bandScore }))
        );
...
export const getIELTSExamById = async (req: Request, res: Response) => {
    try {
        ...
        const exam = await IELTSExam.findById(id).populate(
            "createdBy",
            "fullName email"
        );
        ...
        if (type === "reading" && passages) {
            totalQuestions = passages.reduce(
                (total: number, passage: any) =>
                    total + (passage.questions ? passage.questions.length : 0),
                0
            );
    ...
    try {
        ...
        if (updateData.passages || updateData.sections) {
            ...
            if (updateData.type === "reading" && updateData.passages) {
                totalQuestions = updateData.passages.reduce(
                    (total: number, passage: any) =>
                        total +
                ...
                totalQuestions = updateData.sections.reduce(
                    (total: number, section: any) =>
                        total +
...
export const getExamStats = async (req: Request, res: Response) => {
    try {
        const [totalExams, readingExams, listeningExams, publishedExams] =
            await Promise.all([
...
export const getTestResultDetail = async (
    req: Request,
    res: Response
): Promise<void> => {
    try {
