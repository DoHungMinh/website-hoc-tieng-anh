  ...
  try {
    ...
    if (!progress) {
      return res.status(404).json({
        success: false,
        message: 'Chưa có dữ liệu tiến độ. Hãy bắt đầu học để tạo tiến độ!'
      });
    }
...
export const updateVocabularyProgress = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const existingWord = progress.vocabulary.recentWords.find(w => w.word.toLowerCase() === word.toLowerCase());
    
    if (existingWord) {
      // Update existing word
      existingWord.reviewCount += 1;
      existingWord.masteryLevel = Math.min(100, existingWord.masteryLevel + 5);
    } else {
      // Add new word
      const newWord = {
        word,
      ...
      if (progress.vocabulary.recentWords.length > 50) {
        progress.vocabulary.recentWords = progress.vocabulary.recentWords.slice(-50);
      }
    }
...
export const updateListeningProgress = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user.id;
    ...
    if (progress.listening.recentSessions.length > 20) {
      progress.listening.recentSessions = progress.listening.recentSessions.slice(-20);
    }
    ...
    // Check achievements
    await checkListeningAchievements(userId, progress.listening.hoursCompleted);
...
export const updateTestProgress = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!testName || score === undefined || !maxScore || percentage === undefined) {
      return res.status(400).json({
        success: false,
    ...
    if (progress.testsCompleted.recentTests.length > 15) {
      progress.testsCompleted.recentTests = progress.testsCompleted.recentTests.slice(-15);
    }
    ...
    // Check achievements
    await checkTestAchievements(userId, progress.testsCompleted.completed, percentage);
...
const updateStudyActivity = async (userId: string, activity: string, hours: number) => {
  try {
    const progress = await Progress.findOne({ userId });
    if (!progress) return;
    ...
    let currentWeek = progress.weeklyActivity.find(w => w.week === weekString);
    
    if (!currentWeek) {
      currentWeek = {
        week: weekString,
        days: dayNames.map(day => ({ day, hours: 0, activities: [] })),
        totalHours: 0
      };
    ...
    const currentDayActivity = currentWeek.days.find(d => d.day === currentDay);
    ...
    currentWeek.totalHours = currentWeek.days.reduce((sum, day) => sum + day.hours, 0);
...
const checkVocabularyAchievements = async (userId: string, wordsLearned: number) => {
  const progress = await Progress.findOne({ userId });
  if (!progress) return;

  const achievements = [
    { id: 'vocab_10', title: 'Khởi đầu từ vựng', description: 'Học được 10 từ vựng đầu tiên', icon: '📖', threshold: 10 },
    { id: 'vocab_50', title: 'Người học từ vựng', description: 'Học được 50 từ vựng', icon: '📚', threshold: 50 },
    { id: 'vocab_100', title: 'Học thuộc 100 từ', description: 'Học được 100 từ vựng mới', icon: '📚', threshold: 100 },
    { id: 'vocab_500', title: 'Chuyên gia từ vựng', description: 'Học được 500 từ vựng', icon: '🎓', threshold: 500 },
    { id: 'vocab_1000', title: 'Bậc thầy từ vựng', description: 'Học được 1000 từ vựng', icon: '👑', threshold: 1000 }
  ];
...
const checkListeningAchievements = async (userId: string, hoursCompleted: number) => {
  const progress = await Progress.findOne({ userId });
  if (!progress) return;

  const achievements = [
    { id: 'listening_5', title: 'Người nghe mới', description: 'Hoàn thành 5 giờ luyện nghe', icon: '🎧', threshold: 5 },
    { id: 'listening_20', title: 'Người nghe chăm chỉ', description: 'Hoàn thành 20 giờ luyện nghe', icon: '🎧', threshold: 20 },
    { id: 'listening_50', title: 'Người nghe chuyên nghiệp', description: 'Hoàn thành 50 giờ luyện nghe', icon: '🎧', threshold: 50 },
    { id: 'listening_100', title: 'Bậc thầy nghe', description: 'Hoàn thành 100 giờ luyện nghe', icon: '👂', threshold: 100 }
  ];
...
const checkTestAchievements = async (userId: string, testsCompleted: number, lastScore: number) => {
  const progress = await Progress.findOne({ userId });
  if (!progress) return;

  const achievements = [
    { id: 'test_1', title: 'Bước đầu tiên', description: 'Hoàn thành bài test đầu tiên', icon: '🎯', threshold: 1 },
    { id: 'test_5', title: 'Người làm test', description: 'Hoàn thành 5 bài test', icon: '📝', threshold: 5 },
    { id: 'test_10', title: 'Bậc thầy kiểm tra', description: 'Hoàn thành 10 bài kiểm tra', icon: '🏆', threshold: 10 },
    { id: 'test_25', title: 'Chuyên gia test', description: 'Hoàn thành 25 bài test', icon: '🥇', threshold: 25 }
  ];

  // Score-based achievements
  if (lastScore >= 90) {
    const excellentExists = progress.achievements.find(a => a.id === 'score_excellent');
    if (!excellentExists) {
      progress.achievements.push({
        id: 'score_excellent',
...
const checkStreakAchievements = async (userId: string, currentStreak: number) => {
  const progress = await Progress.findOne({ userId });
  if (!progress) return;

  const achievements = [
    { id: 'streak_3', title: 'Khởi đầu tốt', description: 'Học liên tục trong 3 ngày', icon: '🔥', threshold: 3 },
    { id: 'streak_7', title: 'Streak 7 ngày', description: 'Học liên tục trong 7 ngày', icon: '🔥', threshold: 7 },
    { id: 'streak_14', title: 'Hai tuần kiên trì', description: 'Học liên tục trong 14 ngày', icon: '💪', threshold: 14 },
    { id: 'streak_30', title: 'Tháng hoàn hảo', description: 'Học liên tục trong 30 ngày', icon: '👑', threshold: 30 }
  ];
