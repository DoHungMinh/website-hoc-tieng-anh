...
class PayOSService {
    ...
    async createPaymentLink(orderData) {
        try {
            const { courseId, courseName, price, userId, userEmail } =
                orderData;
            ...
            try {
                ...
                if (course && user) {
                    ...
                    console.log(
                        "💾 Đã lưu PaymentHistory:",
                        paymentLinkResponse.orderCode
                    );
                }
            } catch (historyError) {
                console.warn(
                    "⚠️ Không thể lưu PaymentHistory (không ảnh hưởng payment):",
    ...
    async getPaymentStatus(orderCode) {
        try {
            ...
            console.error(
                `❌ Lỗi kiểm tra trạng thái payment ${orderCode}:`,
                error
            );
