...
// Webhook URL - sử dụng ngrok URL của bạn
const webhookUrl = 'https://5629c587203d.ngrok-free.app/api/payos/webhook';
...
axios.get(webhookUrl, {
    ...
    if (error.response && error.response.status === 405) {
        console.log('✅ Webhook endpoint accessible (405 Method Not Allowed is expected for GET request)');
    } else {
...
axios.post(
    ...
    if (error.response) {
        ...
        if (error.response.status === 400) {
            console.log('💡 Có thể webhook URL đã được đăng ký rồi hoặc không hợp lệ');
        }
    } else if (error.code === 'ECONNABORTED') {
        console.error('⏰ Timeout - PayOS không thể kết nối tới webhook URL');
        console.log('💡 Kiểm tra lại:');
        console.log('   1. Ngrok có đang chạy không?');
        console.log('   2. Backend server có đang chạy trên port 5002 không?');
